...
int main()
{
    ...
    for (int i = 0; i < 40000; ++i)
    {
        ...
        if (i % 100 != 0 || i == 0)
        {
            h.insertItemQuadratic(testData[i]);
        }
        else
        {
            h.insertItemQuadratic(testData[i]);
    ...
    for (int k = 0; k < 40000; ++k)
    {
        ...
        if (k % 100 != 0 || k == 0)
        {
            h.searchItemQuadratic(randNums[k]);
        }
        else
        {
            h.searchItemQuadratic(randNums[k]);
    ...
    std::fstream fout;
    fout.open("insert_search_performance_hash_quadratic_dataSetB.csv", std::ios::out | std::ios::app);
    for (int h = 0; h < 400; ++h)
    {
        if (h == 0)
            fout << "Insert Times\n";
