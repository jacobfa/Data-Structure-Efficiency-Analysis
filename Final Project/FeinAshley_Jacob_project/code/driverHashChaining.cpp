...
int main()
{
    ...
    for (int i = 0; i < 40000; ++i)
    {
        ...
        if (i % 100 != 0 || i == 0)
        {
            h.insertItemChaining(testData[i]);
        }
        else
        {
            h.insertItemChaining(testData[i]);
    ...
    for (int k = 0; k < 40000; ++k)
    {
        ...
        if (k % 100 != 0 || k == 0)
        {
            h.searchItemChaining(randNums[k]);
        }
        else
        {
            h.searchItemChaining(randNums[k]);
    ...
    std::fstream fout;
    fout.open("insert_search_performance_hash_chaining_dataSetB.csv", std::ios::out | std::ios::app);
    for (int h = 0; h < 400; ++h)
    {
        if (h == 0)
            fout << "Insert Times\n";
